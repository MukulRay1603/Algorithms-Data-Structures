...
public:
            ...
            for(const auto& w_v_paths_pair : all_w_vs_paths){
                ...
                fora(k, 0, w_v_paths.size()){
                    ...
                    fora(w, i+1, w_v_paths.size()){
                        ...
                        int p = min_avg_coverage_kmer(w_v_paths[i], w_v_paths[j], src, des, i, j);
    ...
    int min_avg_coverage_kmer(const BubblePath& bpath1, const BubblePath&  bpath2, int src, int des, int i, int j){
        const vector<int>& vpath1 = bpath1.path;
        const vector<int>& vpath2 = bpath2.path;

        int d1 = vpath1.size() - 1;
        int d2 = vpath2.size() - 1;
        ...
        fora(v, 0, vpath1.size())
            if(!disjoint) break;
            else if(vpath1[v] != src && vpath1[v] != des)
                if(spath.find(v) != spath.end()) {
                    disjoint = false;
                    common_ancestor = vpath1[v];
                    cap1 = v;
                    break;
                }

        //if they not disjoint find the position of the ancestor in both paths
        if(!disjoint){
            fora(i, 0, vpath2.size()) if(vpath2[i] == common_ancestor) {cap2 = i; break;}
            d1 = vpath1.size() - cap1;
            d2 = vpath2.size() - cap2;
        }

        //find the one with max average kmer coverage
        double avg_cov_kmer1 = ((double) bpath1.kmerSum) / d1;
        double avg_cov_kmer2 = ((double) bpath2.kmerSum) / d2;
